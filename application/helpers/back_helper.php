...
function admin_yes_no($value,$yes=1){
	if($value==$yes){
		return '<img src="a/i-yes.gif" />';
	}
	return '<img src="a/i-no.gif" />';
}
...
function admin_moveup($url,$img='a/i-up.gif'){
	return '<a href="'.$url.'"><img src="'.$img.'" /></a>';
}

function admin_movedown($url){
	return admin_moveup($url,'a/i-down.gif');
}
...
EOT;
	...
	if(is_array($q)&&count($q)>0){
		...
		foreach($photo as $pic=>$v){
			...
			$ret.=<<<EOT
<li><img src="$tmb" /><em><a href="$pic" rel="popup">查看</a><a href="index.php?uploadify/delete/$tmbx" rel="delete_img">删除</a></em></li>
EOT;
			if($from_post){
...
EOT;
	...
	if(is_array($q)&&count($q)>0){
		foreach($q as $file){
			...
			$ret.=<<<EOT
<li><img src="a/doc.gif" /><em><a href="$file" target="_blank">下载</a><a href="index.php?uploadify/delete/$filex" rel="delete_img">删除</a></em></li>
EOT;
			if($from_post){
...
EOT;
	...
	if(is_array($q)&&count($q)>0){
		foreach($q as $file){
			$filex=admin_encode_segment($file);
			$ret.=<<<EOT
<li><img src="a/flv.gif" /><em><a href="flv.php?$filex" target="_blank">播放</a><a href="index.php?uploadify/delete/$filex" rel="delete_img">删除</a></em></li>
EOT;
			if($from_post){
				$ret.='<input type="hidden" name="'.$tag.'_list[]" value="'.$file.'" style="display:none;" />';
			}
