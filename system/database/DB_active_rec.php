...
class CI_DB_active_record extends CI_DB_driver {
	...
	function _like($field, $match = '', $type = 'AND ', $side = 'both', $not = '')
	{
		...
		//mod by visvoy: fix "or where" bug
		$no_other_where=(count($this->ar_where)<1&&count($this->ar_wherein)<1);
		
		foreach ($field as $k => $v)
		{
			$k = $this->_protect_identifiers($k);

			$prefix = (count($this->ar_like) == 0&&$no_other_where) ? '' : $type;
			//end of mod
	...
	function get($table = '', $limit = null, $offset = null)
	{
		...
		if (!is_null($limit)&&$offset>0&&!$this->_support_limit&&is_object($result))
		{
			$result->first_seek=$offset;
		}
	...
	/**
		if (count($this->ar_like) > 0)
		{
			//mod by visvoy: fix "or where" bug
			$t=implode("\n", $this->ar_like);
			if (count($this->ar_where) > 0&&substr($t,0,4)!='AND '&&substr($t,0,3)!='OR ')
			{
				$sql .= "\nAND ";
			}else{
				$sql .= "\n";
