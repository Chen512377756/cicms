...
class CI_Image_lib {
	...
	function clear()
	{
		...
		$this->init_width=$this->init_height=$this->fill_border='';
	...
	function initialize($props = array())
	{
		...
		// process fill border feature init
		if(!empty($this->width)&&!empty($this->height)){
			$this->init_width=$this->width;
			$this->init_height=$this->height;
		}
		if(!empty($this->fill_border)){
			$def='white';
			if(is_string($this->fill_border)){
				$this->fill_border=trim(strtolower($this->fill_border));
				if(!isset($this->_border_rgb[$this->fill_border])){
					$this->fill_border=$def;
				}
				$this->fill_border=$this->_border_rgb[$this->fill_border];
			}else if(!is_array($this->fill_border)||count($this->fill_border)!=3){
				$this->fill_border=$this->_border_rgb[$def];
			}
		}
	...
	function image_process_gd($action = 'resize')
	{
		...
		//mod by visvoy
		$x_pos=$y_pos=0;
		$dst_img = $create($this->init_width, $this->init_height);
		$dst_bg = imagecolorallocate($dst_img
			,$this->fill_border[0]
			,$this->fill_border[1]
			,$this->fill_border[2]);
		imagefill($dst_img,0,0,$dst_bg);
		
		if($this->width<$this->init_width){
			$x_pos=intval(($this->init_width-$this->width)/2);
		}
		if($this->height<$this->init_height){
			$y_pos=intval(($this->init_height-$this->height)/2);
		}
		...
		$copy($dst_img, $src_img, $x_pos, $y_pos, $this->x_axis, $this->y_axis, $this->width, $this->height, $this->orig_width, $this->orig_height);
