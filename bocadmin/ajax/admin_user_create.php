...
<ul class="halfside">
	<li>帐号<br />
		<?=form_new_input('account','','class="text m"')?></li>
	<li>权限<br />
	...
	<i class="clear"></i>
	<li>密码<br />
		<?=form_new_password('password','','class="text m"')?></li>
	<li>重复密码<br />
		<?=form_new_password('passconf','','class="text m"')?></li>
</ul>
</div>
<table class="datable" border="0" cellpadding="0" cellspacing="0" height="100%">
	...
	<?php 
		...
		<tr><td><?php echo strpos($r,'|-')?$r:'<strong>'.$r.'</strong>'?></td>
			<td><?php if($arr_types[$k_id]['is_havechild']==0){
			$str_use='';
			if($arr_types[$k_id]['is_use']==1){
				$str_use=' disabled="disabled"';
			}
			echo form_checkbox('all_'.$k_id,1,false,'id="all_'.$k_id.'" onclick="check_all('.$k_id.');"'.$str_use); ?><label for="<?php echo 'all_'.$k_id; ?>"><strong>全部</strong></label>&nbsp;&nbsp;&nbsp;&nbsp;
			<?php foreach($auth_power as $k=>$v){
			...
			echo form_checkbox('all_'.$a[$i],1,false,'id="all_'.$a[$i].'" onclick="check_all('.$a[$i].');"'.$str_use); ?><label for="<?php echo 'all_'.$a[$i]; ?>"><strong>全部</strong></label>&nbsp;&nbsp;&nbsp;&nbsp;
			<?php foreach($auth_power as $k=>$v){
