...
$(function(){
	$("input[rel=index]").focus(function(){
		...
		$.getJSON("network.php?jsChangeIndex/"+id+"/"+val,function(json){
...
<table class="datable" border="0" cellpadding="0" cellspacing="0" height="100%">
	...
	<?php foreach($data as $r){?>
		<tr rel="<?=$r->id?>" value="">
			...
			<td><?=isset($type_tree[$r->city_id])?$type_tree[$r->city_id]:$r->city_id?></td>
			<td><a href="<?=site_url("$title/edit/$type_id/{$r->id}/$page/$order/$each/$cond")?>" title="点击编辑" rel="ajax"><?=strcut($r->title,16)?></a></td>
			<td><?=$r->tel?></td>
			<td><?=$r->fax?></td>
			<td><?=strcut($r->addr,10)?></td>
			<td><?=$r->url?></td>
