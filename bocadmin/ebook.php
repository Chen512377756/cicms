...
$(function(){
	$("input[rel=index]").focus(function(){
		...
		$.getJSON("ebook.php?jsChangeIndex/"+id+"/"+val,function(json){
...
<table class="datable" border="0" cellpadding="0" cellspacing="0" height="100%">
	...
	<?php foreach($data as $r){?>
		<tr rel="<?=$r->id?>" value="">
			...
			<td><a href="<?=site_url("$title/edit/$type_id/{$r->id}/$page/$order/$each/$cond")?>" title="点击编辑" rel="ajax"><?=strcut($r->title,22)?></a></td>
			<td><?=admin_toggle(site_url("$title/toggle/$r->id/show"),$r->show)?></td>
			<td><a href="<?=site_url("$title/child/{$r->id}")?>">管理图片</a> | <a href="<?=site_url("$title/edit/$type_id/{$r->id}/$page/$order/$each/$cond")?>" rel="ajax">编辑</a> | <a rel="delete" href="javascript:js_delete(<?=$r->id?>,'<?=site_url("$title/delete/$r->id")?>');">删除</a></td>
		</tr>
	<?php }?>
	</tbody>
</table>
