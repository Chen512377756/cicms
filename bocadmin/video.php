...
$(function(){
	$("input[rel=index]").focus(function(){
		...
		$.getJSON("video.php?jsChangeIndex/"+id+"/"+val,function(json){
...
<div class="bdiv">
	<div class="btop">
		...
		<h2><a href="<?=site_url("$title/index/$type_id")?>"><?=$caption?></a><em><a href="<?=site_url("$title/create/$type_id")?>" rel="ajax"><img src="a/i-add.gif" align="absmiddle" />添加</a></em></h2>
...
<table class="datable" border="0" cellpadding="0" cellspacing="0" height="100%">
	...
	<?php foreach($data as $r){?>
		<tr rel="<?=$r->id?>" value="">
			...
			/**if(!empty($r->url)){
				$url='flv.php?outer/'.admin_encode_segment($r->url);
			}else{
				$url='flv.php?'.admin_encode_segment($r->video);
			}**/
			if(!empty($r->url)){
				$url='index.php?flv/outer/'.admin_encode_segment($r->url);
			}else{
				$url='index.php?flv/index/'.admin_encode_segment($r->video);
			}
			?><a href="<?=$url?>" title="<?=htmlspecialchars($r->title)?>" target="_blank"><img src="<?=$pic?>" width="50" height="50" /></a></td>
			<td><a href="<?=site_url("$title/edit/$type_id/{$r->id}/$page/$order/$each/$cond")?>" title="点击编辑" rel="ajax"><?=strcut(strip_tags(nl2br($r->title)),22)?></a></td>
