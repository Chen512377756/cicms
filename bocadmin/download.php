...
$(function(){
	$("input[rel=index]").focus(function(){
		...
		$(this).parent().append("<img src='a/loading.gif' style='width:16px;height:16px;' align='absmiddle' />");
            $.getJSON("download.php?jsChangeIndex/"+id+"/"+val,function(json){
...
<table class="datable" border="0" cellpadding="0" cellspacing="0" height="100%">
	...
	<?php foreach($data as $r){?>
		<tr rel="<?=$r->id?>" value="">
			...
			<td><a href="<?=site_url("$title/edit/$type_id/{$r->id}/$page/$order/$each/$cond")?>" title="点击编辑" rel="ajax"><?=strcut($r->title,40)?></a></td>
			<td><?=admin_toggle(site_url("$title/toggle/$r->type_id/$r->id/show"),$r->show)?></td>
			<!--<td><? //=admin_toggle(site_url("$title/toggle/$r->type_id/$r->id/recommend"),$r->recommend)?></td>-->
...
<ul>
	...
	<li><a href="<?=site_url("$title/searchebook/$type_id")?>" class="button" rel="ajax"><span>搜索</span></a></li>
</ul>
<div class="page right"><?=$page_link?></div>
<div class="i10"></div>
</div>
