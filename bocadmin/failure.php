...
<div class="bdiv">  
	<div class="btop">
		<div class="right"><?=lang('site_title')?>&nbsp;</div>
		<h2>操作失败</h2>
	</div>
	...
	<div class="dialog-bottom">
		<?php if(empty($url)){?>
		<div class="dbright">
			<a href="javascript:history.go(-1);" class="button"><span>&nbsp; 上一页 &nbsp;</span>
		</div>
		<?php }else{?>
		<div class="dbleft">
			<a href="javascript:history.go(-1);" class="button"><span>&nbsp; 上一页 &nbsp;</span></a>
		</div>
		<div class="dbright">
			<?php if(!is_array($url)){?>
			...
			<?php }else foreach($url as $k=>$v){
				...
				?>
				<a href="<?=$v?>" <?=$x?> class="button"><span>&nbsp; <?=$k?> &nbsp;</span></a>
			<?php }?>
		</div>
		<?php }?>
		<div class="clear"></div>
