...
<div class="bdiv">
	<div class="btop">
		<h2><a href="<?=site_url("$title/index")?>"><?=$caption?></a><em><?=$total?>个符合&nbsp;&nbsp;<a href="<?=site_url("$title/create")?>" rel="ajax"><img src="a/i-add.gif" align="absmiddle" />添加</a></em></h2>
...
<table class="datable" border="0" cellpadding="0" cellspacing="0" height="100%">
	...
	<?php foreach($data as $r){?>
		<tr rel="<?=$r->id?>" value="">
			...
			<td><?php if($r->is_fixed!=1){ ?><a href="<?=site_url("$title/create/{$r->id}")?>" rel="ajax">添加子类</a> | <a href="<?=site_url("$title/edit/{$r->id}")?>" rel="ajax">编辑</a> | <a rel="delete" href="<?=site_url("$title/delete/{$r->id}")?>">删除</a><?php } ?></td>
		</tr>
	<?php }?>
	</tbody>
</table>
