...
<div class="bdiv">  
	...
	<div class="notice">上次<a href="<?=site_url('backup')?>" rel="ajax">数据备份</a>时间：<?=date('Y-m-d H:i:s',$databkup_timestamp)?></div>
	<?php }?>
	<div class="bmain box-wrap">  
		<p>网站名称：<a href="../" target="_blank"><?=lang("site_title")?></a></p>
		<p>平台版本：v<?=CI_VERSION?></p>
		<p>引擎类型：PHP v<?=phpversion()?></p>
		<p>数据库：<?=ucfirst($this->db->platform()).' v'.$this->db->version()?></p>
...
<div class="bdiv">  
	...
	<?php }?>
	<div class="bmain box-wrap">
		<p>当前帐号：<?=$admin->account?></p>
		<p>帐号权限：<?=$admin->role?></p>
		<p>登录IP：<?=$admin->login_ip?></p>
		<p>登录次数：<?=$admin->login_count?></p>
		<?php if(!isset($default_password_notice))echo '<p>&nbsp;</p>'?>
		<p class="more"><a href="<?=site_url('password')?>" rel="ajax">修改密码&raquo;</a>	</div>  
</div>  
<b class="b3b"></b><b class="b2b"></b><b class="b1b"></b>  
</div>
<div class="clear"></div>
...
<div class="bdiv">  
	...
	<div class="bmain box-wrap" style="height:116px;">  
		<p>网站域名：<?=$server_name?></p>
		<p>网站IP：<?=$server_ip?></p>
		<p>当前编码：<?=$sys_charset?></p>
		<p>缓存类型：<?=$server_htmlize?'静态化':'页面'?></p>
		<p>网站日志：<?=admin_yes_no($server_log)?></p>
	</div>  
</div>  
<b class="b3b"></b><b class="b2b"></b><b class="b1b"></b>  
</div>
...
<div class="bdiv">  
	...
	<div class="bmain box-wrap" style="height:116px;">  
		<p>时区设置：<?=$server_timezone?></p>
		<p>上传上限：<?=$sys_max_upload?></p>
		<p>安全模式：<?=admin_yes_no($sys_safe_mode)?></p>
		<p>socket支持：<?=admin_yes_no($sys_socket)?></p>
		<p>zlib支持：<?=admin_yes_no($sys_zlib)?></p>
	</div>  
</div>  
<b class="b3b"></b><b class="b2b"></b><b class="b1b"></b>  
</div>
